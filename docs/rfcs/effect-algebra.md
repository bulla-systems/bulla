...
**Cross-cutting**, like [the surface conventions](surface-conventions.md), and
underneath [verified effect rows](verified-effect-rows.md): that document makes
...
[the conflict table](verified-effect-rows.md#the-concurrency-consequence) is a
...
[the three-row table](shared-state-invariants.md#the-conflict-rule-gains-three-rows),
now with a reason.
...
[needs masking rather than ordering](shared-state-invariants.md#a-lock-a-handler-takes-needs-masking-not-ordering),
...
combinations is an RFC; arbitrary equations are a research rung.
...
[the ladder](../the-ladder.md#the-rungs) beside noninterference and cost: surface
...
|---|---|
| [verified effect rows](verified-effect-rows.md) | its conflict table, derived rather than stipulated; the containment order refines the instance relation |
| [shared-state invariants](shared-state-invariants.md) | `owns(r)` as dynamically-established commutation |
| [resource types](resource-types.md) | `forgets(r)` as an ordinary state effect |
| [protocol types](protocol-types.md) | `blocks` as a given atom, recorded and unproved |
| [the surface conventions](surface-conventions.md) | the two-family shape, which is this basis seen from the surface |
