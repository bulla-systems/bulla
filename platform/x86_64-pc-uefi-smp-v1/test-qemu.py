...
# Bulla divergence from Thermite 84d276e7 (docs/upstream-pin.md): the upstream
# roots are the three Linux distribution layouts. On a host where none of those
# paths exist and none can be created -- a macOS host under SIP, or a Nix or
# Homebrew prefix -- the gate cannot find firmware and cannot run at all. This
# prepends a root named by BULLA_OVMF_DIR. It changes where firmware is found
# and nothing else: the pairs, the QEMU invocation, and every transcript
# assertion below are untouched, so a gate that passes here passes upstream.
FIRMWARE_ROOTS = tuple(
    Path(root) for root in (os.environ.get("BULLA_OVMF_DIR"),) if root
) + (
